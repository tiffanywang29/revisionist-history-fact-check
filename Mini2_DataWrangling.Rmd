...
colleges_food_rank <- colleges_food_rank %>%
  mutate(food_rank = as.numeric(rank), forbes_rank = as.numeric(forbes_rank)) %>%
...
ggplot(colleges_food, aes(x = par_ed_pct_1stgen, y = pctpell, color = as.factor(ranked))) + 
  geom_point()

#What if we just look at school categories?
private_food <- colleges_food
private_food <- colleges_food %>%
  filter(!is.na(pctpell), !is.na(par_ed_pct_1stgen)) %>%
  group_by(control, ranked) %>%
  summarise(mean_pctpell = mean(pctpell), mean_par_ed_pct_1stgen = mean(par_ed_pct_1stgen))

ggplot(private_food, aes(x = mean_par_ed_pct_1stgen, y = mean_pctpell, color = as.factor(ranked))) + 
  geom_point()
